...
//int interpretar_cmd(cmdIR_t cmd, byte* intersecciones){
	int retval = 0;
	int actual, prev, sig;
...
//					retval = 0;
				...
				if(act_x == sig_x){
//					intersecciones = abs(sig_y - act_y);
					if(prev_x == act_x)
						if((prev_y < act_y && sig_y < act_y) || (prev_y > act_y) && (sig_y > act_y))
							retval = U_TURN;
						else 
							retval = DERECHO;
...
//					intersecciones = abs(sig_x - act_x);
					if(prev_y == act_y)
						if((prev_x < act_x && sig_x < act_x) || (prev_x > act_x) && (sig_x > act_x))
							retval = U_TURN;
						else
							retval = DERECHO;
...
//				state = START;
//				count = 0;
//				error_occured(ERR_NOISE_IN_CMD);
				error = TRUE;
			}
			break;
